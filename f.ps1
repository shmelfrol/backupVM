...
function ExportVM ($vm, $fullpath, $LOG){
    ...
    #проверка бекапа
       if(Test-Path "$fullpath\$vm\Virtual Hard Disks\*.vhdx"){
           Send-MailMessage -From 'it-events@xxx.ru' -To 'it-events@xxx.ru' -Subject "$(get-date -format "dd.MM.yy.HH.mm.ss") Backup $vm OK" -Body "Backup $vm OK" ЦSmtpServer 'xxxx.ru' -Encoding 'UTF8'
           write "$(get-date -format "dd.MM.yy.HH.mm.ss") 'экспорт $vm в $fullpath OK" | out-file $LOG -append
       }else{
           Send-MailMessage -From 'it-events@xxx.ru' -To 'it-events@xxx.ru' -Subject "$(get-date -format "dd.MM.yy.HH.mm.ss") Backup $vm ERROr" -Body "Backup $vm ERROR" ЦSmtpServer 'xxxx.ru' -Encoding 'UTF8'
