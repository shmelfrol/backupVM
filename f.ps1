...
function ExportVM ($vm, $fullpath, $LOG){
      ...
      #проверка бекапа
       if(Test-Path "$fullpath\$vm\Virtual Hard Disks\*.vhdx"){
           ...
           Send-MailMessage -From 'it-events@kubstu.ru' -To 'it-events@kubstu.ru' -Subject "$(get-date -format "dd.MM.yy.HH.mm.ss") Backup $vm OK ($duration min, $FolderSize Gb)" -Body "Backup $vm OK ($duration min), VM Size - $FolderSize Gb" ЦSmtpServer 'mail.kubstu.ru' -Encoding 'UTF8'
           write "$(get-date -format "dd.MM.yy.HH.mm.ss") 'экспорт $vm в $fullpath OK" | out-file $LOG -append
       }else{
           Send-MailMessage -From 'it-events@kubstu.ru' -To 'it-events@kubstu.ru' -Subject "$(get-date -format "dd.MM.yy.HH.mm.ss") Backup $vm ERROr" -Body "Backup $vm ERROR" ЦSmtpServer 'mail.kubstu.ru' -Encoding 'UTF8'
