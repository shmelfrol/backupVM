...
write "$(get-date -format "dd.MM.yy.HH.mm.ss") папка для бекапов сейчас имеет размер: $FolderSize" | out-file  $LOG -append
...
While ($q -ne $R){
#обнуляем переменную в которую записываем количество подпапок в папке для бекапв
$q=0
$colpap = dir $folder
$datetime = get-date
    foreach ($p in $colpap ){
...
Write "$q !!!!"
Write "$R !!!!"
#если количество папок в папке для бекапов меньше или равно требуемого количества бекапов то выходим из цикла
if ($q -le $R){break}
    $colpap = dir $folder
    foreach ($s in $colpap ){
        if ($q -gt $R){
...
function SetVMStatus ($vmoff, $vm, $LOG){
        ...
        if ($vmoff -eq 1){
                if ($vmstate.state -eq "Running"){
                write "$(get-date -format "dd.MM.yy.HH.mm.ss") Сохраняем  $vm" | out-file $LOG -append
...
if ($cn.name -eq $VmName){
   write "$(get-date -format "dd.MM.yy.HH.mm.ss") Обнаружена $VmName на $i" | out-file $LOG -append
   if ($i -eq $blade){
       write "$(get-date -format "dd.MM.yy.HH.mm.ss") $VmName располагается локально" | out-file $LOG -append
       if ($proverkaPaths= 1){
